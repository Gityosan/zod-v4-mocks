...
- Added `preflightCheck` config option (default `true`) — a pre-flight schema walk that rejects constructs the library cannot safely mock before generation starts. Detects an un-mocked `z.custom()` / `z.instanceof()` at a fixed-length tuple position and throws with the offending path (downgraded to a warning when an opaque `supply`/`override` is registered). Set to `false` to skip
...
### Fixed
- Recursive `z.lazy()` no longer stack-overflows when the `z.lazy()` itself is the recursion anchor (e.g. `const Tree = z.lazy(() => z.object({ children: z.array(Tree) }))`). `z.lazy()` is now tracked by the recursive depth limiter, so any recursive-lazy shape generates and terminates at `recursiveDepthLimit`
