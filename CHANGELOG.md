...
- Added `preflightCheck` config option (default `true`) — a pre-flight schema walk that runs before generation. Error-level findings (an un-mocked `z.custom()` / `z.instanceof()` at a fixed-length tuple position) throw with the offending path. Warning-level findings carry a minimal auto-fix: preflight emits the warning and substitutes a corrected schema so generation proceeds. Currently a recursive `z.lazy()` that is its own recursion anchor (which would otherwise stack-overflow) is auto-substituted with its unwrapped form. Set to `false` to skip both checks and fixes
